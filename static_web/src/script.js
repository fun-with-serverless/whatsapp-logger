...
const languageOptions = [
  ...
  { value: "Mandarin_Chinese", label: "普通话" },
...
async function showQR() {
  try {
    ...
    console.log(error.message);
    alert(
      "Error in loading the QR code. Please check your username and password."
    );
  }
}
...
async function pullGroups() {
  try {
    ...
    const table = new Table("#groups-table", [
      "Group Name",
    ...
    response.groups.forEach((item) => {
      ...
      const $languageDropDown = $("<select>").addClass("form-control");
      ...
      $languageDropDown.val(item.summary_language);
      if (item.summary_status == "None") {
      ...
      $languageDropDown.on("change", function () {
        ...
        getRequest({
          ...
          body: JSON.stringify({ summary_language: selectedValue }),
        });
      });
      const $space = $("<span>").text(" ");
      table.addRow(item.group_id, [
        { value: item.name, isHtml: false },
        { value: $dropdown.add($space).add($languageDropDown), isHtml: true },
      ]);
    });
  } catch (error) {
    console.log(error.message);
...
async function pullStatus() {
  try {
    ...
    $("#last_message_arrived_value").html(
      new Date(response.last_message_arrived).toLocaleString()
    );
